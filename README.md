...
- 当前核心开放问题包括部分 publisher-formatted 论文的访问边界、RZ axis-charge correctness、完整 transition-zone route ledger、RZ implicit Villasenor 的 PETSc KSP construction SIGILL 和公开发行权利。Vay+AMR 已经通过真实初始化 guard 确认是当前上游不支持的配置边界，不再被误列为待补物理运行覆盖。全部证据与关闭条件见 [当前缺口登记](docs/current-book-gap-register.md)。
