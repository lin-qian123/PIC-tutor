...
- [ ] 继续推进 comoving PSATD patch 四件套：`test_2d_galilean_psatd_hybrid` 的本地 single-process control experiment 已证明当前这台机器在 `warpx.numprocs='1 1'` 下仍能复现已知 Galilean unstable-energy contrast（`stable_over_unstable_energy_ratio = 0.8418992628444483`），所以 comoving `no-comoving` sibling 当前不抬高能量，已经不能简单归咎于“缺 MPI”。下一步应先重审 comoving contrast 设计本身：优先查是否需要换 sibling（例如保持 comoving family 语义同时更贴近实际 unstable branch），然后再在可用时用更接近 upstream regression 的 MPI/并行设置复核；若仍不能形成清晰 unstable contrast，则把第 6 章下一步切到 RZ PSATD validation 强判据表，并把 comoving energy gate 明确降级为“待更多样本后再定”。
