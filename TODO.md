...
- [ ] 继续推进 comoving PSATD patch 四件套：优先在更接近 upstream regression 的 MPI/并行设置下重跑 stable/contrast sibling，确认是否能得到比 stable baseline 更高的 non-comoving 电场能量；若仍不能形成清晰 unstable contrast，则把第 6 章下一步切到 RZ PSATD validation 强判据表，并把 comoving energy gate 明确降级为“待更多样本后再定”。
