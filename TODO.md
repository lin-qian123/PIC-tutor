...
- [ ] 继续推进 comoving PSATD patch 四件套：`test_2d_galilean_psatd_hybrid` 的本地 single-process control experiment 已证明当前这台机器在 `warpx.numprocs='1 1'` 下仍能复现已知 Galilean unstable-energy contrast（`stable_over_unstable_energy_ratio = 0.8418992628444483`）；同时 `no-comoving` 与 `no-galilean` sibling 的末态 `electric_energy/e_mag_max/e_mag_p99/spike_ratio` 已在 `1e-14` 相对误差量级内重合，说明两者本机上实际共享同一条 standard-PSATD unstable branch。所以下一步应先重审 comoving gate 语义本身：如果 stable comoving 相对这条共享 unstable branch 并不会降低电场能量，就需要明确把 comoving patch 的第一阶段重心转到 finite/spike 或其他更合适的判据，而不是继续默认沿用 Galilean 式 energy gate；然后再在可用时用更接近 upstream regression 的 MPI/并行设置复核。若仍不能形成清晰 unstable contrast，则把第 6 章下一步切到 RZ PSATD validation 强判据表，并把 comoving energy gate 明确降级为“待更多样本后再定”。
