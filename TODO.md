...
- [x] 2026-06-29：完成 `v0.19` PSATD NCI 策略谱系版：冻结 `manuscript/VERSION-v0.18.md`，把当前版本说明切到 `manuscript/VERSION.md` 的 v0.19，新增 `scripts/build_v19.py`，生成 `dist/pic-tutor-v0.19.md` 与 `dist/pic-tutor-v0.19.html`；新增 Godfrey et al. 2014 论文专属目录，保存 PDF、MinerU Markdown、图片、中文讲解笔记和 reading log；第 6 章新增 `6.6.3 v0.19 文献闭环：Godfrey et al. 2014 的 PSATD NCI 策略谱系`，把 fixed-grid PSATD 的 filtering、higher-order interpolation、current scaling、timestep selection 与 Galilean 表示层面策略分开说明，并连接 `warpx.use_filter`、`psatd.current_correction`、`psatd.do_time_averaging` 和 `psatd.JRhom` 的不同机制边界。
- [x] 2026-06-29：完成 `v0.18` boosted-frame Galilean 速度闭环版：冻结 `manuscript/VERSION-v0.17.md`，把当前版本说明切到 `manuscript/VERSION.md` 的 v0.18，新增 `scripts/build_v18.py`，生成 `dist/pic-tutor-v0.18.md` 与 `dist/pic-tutor-v0.18.html`；新增 Kirchen et al. 2016 论文专属目录，保存 PDF、MinerU Markdown、图片、中文讲解笔记和 reading log；第 6 章新增 `6.6.2 v0.18 文献闭环：Kirchen et al. 2016 的 boosted-frame 应用证据`，连接 boosted-frame 背景等离子体漂移速度、Galilean 网格速度、`psatd.use_default_v_galilean` 语义和 NCI regression/application evidence。
...
- [x] 为 v0.18 继续推进第 6 章文献闭环：选择 Kirchen POP 2016 或 Godfrey PSATD/NCI 论文，补 MinerU Markdown、图片、中文讲解笔记，并把 time averaging、boosted-frame 默认 Galilean 速度或 NCI filter 线索接回源码与 regression。
- [x] 为 v0.19 继续推进第 6 章 NCI 策略谱系：选择 Godfrey PSATD/NCI 论文，补 MinerU Markdown、图片、中文讲解笔记，并把 filtering、`E/B` correction、current scaling 与 Galilean 表示的差异接回 WarpX 参数和 regression。
...
- [x] 2026-06-30：完成 `v0.32` comoving PSATD local reference calibration audit 版：冻结 `manuscript/VERSION-v0.31.md`，把当前版本说明切到 `manuscript/VERSION.md` 的 v0.32，新增 `scripts/build_v32.py`，生成 `dist/pic-tutor-v0.32.md` 与 `dist/pic-tutor-v0.32.html`；补入 `runs/fieldsolver-validation/comoving-stable-baseline`、`comoving-unstable-no-comoving` 的本地单进程运行，生成 `runs/fieldsolver-validation/comoving-reference-ledgers/comoving-stable-baseline.{md,json}` 与 `comoving-stable-vs-no-comoving.{md,json}`；确认 `psatd.use_default_v_comoving=0, psatd.v_comoving=0` 确实切到非 comoving 分支，但当前单进程样本的 `stable_over_unstable_energy_ratio = 1.0469608718245416`、`spike_ratio_ref_stable = 1.1103719982074416` 说明它还不足以直接充当最终 CI `energy_ref_unstable`，因此本版把结论收口为 local calibration audit，而不是直接声称 patch gate 已定。
- [x] 2026-06-30：完成 `v0.33` comoving PSATD first-stage patch asset reproducibility 版：冻结 `manuscript/VERSION-v0.32.md`，把当前版本说明切到 `manuscript/VERSION.md` 的 v0.33，新增 `scripts/build_v33.py`，生成 `dist/pic-tutor-v0.33.md` 与 `dist/pic-tutor-v0.33.html`；新增 `scripts/build_comoving_first_stage_patch.py`，把 `analysis_comoving_first_stage_draft.py` 和 `comoving_first_stage_patch.diff` 收敛为 ledger 驱动重建资产，并用当前 `spike_ratio_ref_stable = 1.1103719982074416` 与默认 `safety_factor = 1.001` 自动回填候选 `SPIKE_RATIO_MAX`，避免后续 helper/diff 手工漂移。
- [ ] 继续推进 comoving PSATD patch 四件套：当前 `PIC-tutor` 已同时具备可配置原型 `scripts/analysis_comoving.py`、最小草案 helper、unified diff，以及可重建它们的 `scripts/build_comoving_first_stage_patch.py`。下一步应先决定这份 draft helper 是否直接上提成 WarpX patch 第一阶段主 gate：如果 stable comoving 相对 shared unstable branch 仍不能给出可靠 energy ordering，就应把 WarpX patch 第一阶段明确收敛到 `finite + spike`，把 energy gate 继续留在 `待更多样本/更好 sibling 后再定`；然后再在可用时用更接近 upstream regression 的 MPI/并行设置复核。若这条线仍停住，则把第 6 章下一步切到 RZ PSATD validation 强判据表。
