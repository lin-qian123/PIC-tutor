...
- [ ] 继续推进 comoving PSATD patch 四件套：当前 `PIC-tutor` 已同时具备可配置原型 `scripts/analysis_comoving.py` 和更接近 WarpX 提交流的最小草案 `notes/code-reading/fieldsolver/analysis_comoving_first_stage_draft.py`，并已在现有样本上验证出“stable 通过、no-comoving sibling 失败”的 `finite + spike` 分辨力。下一步应先决定这份 draft helper 是否直接上提成 WarpX patch 第一阶段主 gate：如果 stable comoving 相对 shared unstable branch 仍不能给出可靠 energy ordering，就应把 WarpX patch 第一阶段明确收敛到 `finite + spike`，把 energy gate 继续留在 `待更多样本/更好 sibling 后再定`；然后再在可用时用更接近 upstream regression 的 MPI/并行设置复核。若这条线仍停住，则把第 6 章下一步切到 RZ PSATD validation 强判据表。
