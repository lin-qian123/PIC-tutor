...
subtitle: "v0.27 PSATD time-averaging Psi/Y 系数闭环版"
author: "PIC-tutor"
date: "2026-06-29"
lang: zh-CN
---
...
`v0.27` 是 `PIC-tutor` 的 PSATD time-averaging `Psi/Y` 系数闭环版。它继承 `v0.26` 的 Cartesian `X1-X4` 源项积分系数图谱，并继续收束第 6 章中一个容易混写的短板：average-field 输出中的 `Psi1/Psi2/Y1-Y4`。

本版仍不修改 `../warpx`。第 6 章新增 `6.6.2 v0.27 time-averaging Psi/Y 系数的源码公式闭环`，并新增 `notes/code-reading/fieldsolver/18-psatd-time-averaging-coefficients.md`。本版的核心写作边界是：这里的 `Psi/Y` 只对应 Cartesian `PsatdAlgorithmGalilean.cpp` 的 average-field 输出，不直接覆盖 JRhom `Y1-Y8`、RZ/Galilean RZ、comoving 或 PML PSATD。

本版仍不是出版终稿。它继续保留 LeeCPC2015 PDF/MinerU 缺口，但不再把项目推进完全卡在授权全文上。后续仍应补齐 Lee/Vay PML 论文全文，同时继续把第 6 章的 JRhom、RZ/Galilean RZ 和 comoving 系数体系逐项拆清。
...
v0.27 只修改 `PIC-tutor` 书稿项目，不修改 `../warpx` 原仓库。本版继续按当前 checkout 核对 `PsatdAlgorithmGalilean.cpp:32-632`、`WarpXEvolve.cpp:998-1007`、`WarpXPushFieldsEM.cpp:349-377`、`Examples/Tests/nci_psatd_stability/analysis_galilean.py`、`inputs_test_2d_galilean_psatd*` 和 `inputs_test_3d_galilean_psatd*`。后续若 WarpX 更新，必须重新校准源码行号、文档段落和测试入口后再发布新版。
...
| 场求解器 | `chapters/06-field-solvers.md` | 已补 Lehe/Kirchen/Godfrey 文献闭环、PSATD/NCI 源码机制对照表，并在 v0.26 增补 Cartesian `X1-X4` 源码公式闭环、在 v0.27 增补 time-averaging `Psi/Y` 源码公式闭环 | 仍需 JRhom `Y*`、RZ/Galilean RZ 和 comoving 系数逐项拆表 |
...
- 冻结 `manuscript/VERSION-v0.26.md`，避免重建 v0.26 时误用 v0.27 版本说明。
- 新增 `scripts/build_v27.py`，生成 `dist/pic-tutor-v0.27.md` 与 `dist/pic-tutor-v0.27.html`。
- 把 `scripts/build_v26.py` 改为读取冻结的 `manuscript/VERSION-v0.26.md`，保持 v0.26 版本说明可复现。
- 新增 `notes/code-reading/fieldsolver/18-psatd-time-averaging-coefficients.md`，覆盖 Cartesian `PsatdAlgorithmGalilean.cpp` 的 `Psi1/Psi2/Y1-Y4` 源码公式、零模处理、average-field 更新式和实空间平均场回填路径。
- 更新 `notes/code-reading/fieldsolver/README.md`，把新图谱纳入 fieldsolver 精读顺序。
- 在第 6 章新增 `6.6.2 v0.27 time-averaging Psi/Y 系数的源码公式闭环`，把 `Psi1/Psi2/Y1-Y4` 与 `<E>/<B>`、`J`、`rho_new/rho_old`、`PSATDScaleAverageFields()` 和 `PSATDBackwardTransformEBavg()` 的作用关系写成正文。
...
- 建立稳定的 Markdown/HTML/PDF 构建流程；v0.27 仍只保证 Markdown/HTML。
...
```bash
python scripts/build_v27.py
```

生成的文件：

- `dist/pic-tutor-v0.27.md`
- `dist/pic-tutor-v0.27.html`（若本机存在 `pandoc`）
