...
subtitle: "v0.29 RZ/Galilean RZ PSATD 系数边界版"
author: "PIC-tutor"
date: "2026-06-29"
lang: zh-CN
---
...
`v0.29` 是 `PIC-tutor` 的 RZ/Galilean RZ PSATD 系数边界版。它继承 `v0.28` 的 JRhom `Y1-Y8` 图谱，并继续收束第 6 章中另一个容易混写的短板：RZ 标准 PSATD 的 `X1-X3/X5-X6` 与 Galilean RZ 的 `X1-X4/Theta2/T_rho`。

本版仍不修改 `../warpx`。第 6 章新增 `6.8.1 v0.29 RZ/Galilean RZ PSATD 系数边界`，并新增 `notes/code-reading/fieldsolver/20-psatd-rz-galilean-rz-coefficients.md`。本版的核心写作边界是：RZ 的 `X*` 系数必须和 Cartesian `X*`、JRhom `Y*`、PML `C*` 按 algorithm class 和 field layout 分开。

本版仍不是出版终稿。它继续保留 LeeCPC2015 PDF/MinerU 缺口，但不再把项目推进完全卡在授权全文上。后续仍应补齐 Lee/Vay PML 论文全文，同时继续把第 6 章的 comoving 系数体系和 RZ validation 边界逐项拆清。
...
v0.29 只修改 `PIC-tutor` 书稿项目，不修改 `../warpx` 原仓库。本版继续按当前 checkout 核对 `PsatdAlgorithmRZ.cpp:19-493`、`PsatdAlgorithmGalileanRZ.cpp:19-364`、`SpectralSolverRZ.cpp`、`SpectralFieldDataRZ.cpp` 和 `notes/code-reading/fieldsolver/08-psatd-rz-hankel.md`。后续若 WarpX 更新，必须重新校准源码行号、文档段落和测试入口后再发布新版。
...
| 场求解器 | `chapters/06-field-solvers.md` | 已补 Lehe/Kirchen/Godfrey 文献闭环、PSATD/NCI 源码机制对照表，并在 v0.26 增补 Cartesian `X1-X4`、v0.27 增补 time-averaging `Psi/Y`、v0.28 增补 JRhom `Y1-Y8`、v0.29 增补 RZ/Galilean RZ 系数边界 | 仍需 comoving 系数逐项拆表，并把 RZ validation 边界继续压实 |
...
- 冻结 `manuscript/VERSION-v0.28.md`，避免重建 v0.28 时误用 v0.29 版本说明。
- 新增 `scripts/build_v29.py`，生成 `dist/pic-tutor-v0.29.md` 与 `dist/pic-tutor-v0.29.html`。
- 把 `scripts/build_v28.py` 改为读取冻结的 `manuscript/VERSION-v0.28.md`，保持 v0.28 版本说明可复现。
- 新增 `notes/code-reading/fieldsolver/20-psatd-rz-galilean-rz-coefficients.md`，覆盖 standard RZ `C/S_ck/X1-X3/X5-X6`、Galilean RZ `X1-X4/Theta2/T_rho`、`Ep/Em` 字段布局、time-averaging 限制、current correction 和防混写边界。
- 更新 `notes/code-reading/fieldsolver/README.md`，把新图谱纳入 fieldsolver 精读顺序。
- 在第 6 章新增 `6.8.1 v0.29 RZ/Galilean RZ PSATD 系数边界`，把 RZ 谱基、standard RZ 实系数、linear-J time averaging `X5/X6` 和 Galilean RZ complex 系数分开说明。
...
```bash
python scripts/build_v29.py
```

生成的文件：

- `dist/pic-tutor-v0.29.md`
- `dist/pic-tutor-v0.29.html`（若本机存在 `pandoc`）
