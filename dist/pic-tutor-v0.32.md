...
因此，更严格的说法应当是：本地 audit 已经验证了参数分支和 ledger 提取流程，但 comoving `no-comoving` sibling 当前没有给出可用的 unstable-energy contrast，这更像是 contrast 设计本身还不够贴近真正的 unstable branch，而不只是并行环境缺失。真正写入 WarpX `analysis_comoving.py` 的 `energy_ref/tol_energy`，仍需在更接近 upstream regression 的 MPI/并行设置下重跑，并且可能需要先重新选择更有解释力的 comoving sibling。
