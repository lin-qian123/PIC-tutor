...
subtitle: "v0.32 comoving PSATD local reference calibration audit 版"
author: "PIC-tutor"
date: "2026-06-30"
lang: zh-CN
---
...
这个结果直接决定了 patch 策略边界。当前 local sample 仍然能提供 `spike_ratio_ref_stable = 1.1103719982074416` 这样的 stable envelope 候选值，也能证明 ledger/provenance 工具链是通的；但它还不能让我们诚实地把 `energy_ref_unstable = 7.786411776882875e+14` 宣称为最终 CI gate，因为这个“unstable reference”并没有在本地样本上表现出比 stable baseline 更高的电场能量。更严格的说法应当是：本地 audit 已经验证了参数分支和 ledger 提取流程，但真正写入 WarpX `analysis_comoving.py` 的 `energy_ref/tol_energy` 仍需在更接近 upstream regression 的 MPI/并行设置下重跑，或进一步寻找更有解释力的 contrast sibling。
