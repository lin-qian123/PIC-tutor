...
当前合订 PDF 为 309 页；本 v0.47 版本在 v0.46 基础上补入 RZ Esirkepov rho-side observable，明确物种分解闭合、integrated-rho drift 观测和 `divE-rho` 诊断边界之间的证据层级。
...
| RZ PICMI secondary emission | `secondary_ion_emission/inputs_test_rz_secondary_ion_emission_picmi.py` | `secondary_ion_emission/analysis.py diags/diag1/`；独立 `scripts/analyze_rz_secondary_ion_emission_contract.py` | 当前 producer 发出预期 `2` 个电子，但最近端点相对距离为 `3.6038% > 2%`；官方 geometry gate 失败，不能写成通过 | EB 刮擦 buffer、Python secondary-emission callback、随机热 kick 与球面 impact-point 回溯 |
