...
当前合订 PDF 为 309 页；本 v0.48 版本在 v0.47 基础上统一收口 RZ secondary-emission 的分辨率敏感性边界，明确默认基线与 refined controls 的不同证据等级。
...
本轮又对这条 signed-vs-absolute 差异做了源码级审计。`scripts/audit_particles_in_pml_analysis_contract.py` 对上游 `analysis_particles_in_pml.py` 的 `max(Ex_array.max(), Ey_array.max(), Ez_array.max())`、阈值断言和缺少 `abs()` 的事实逐项通过，同时确认项目独立脚本确实执行 `np.max(np.abs(...))`。因此 3D AMR 的 `Ex_min=-110.3993781372607` 不是读取器把符号弄反，而是上游 consumer 没有把负向峰值纳入 gate。最小上游修正建议是把官方 consumer 改成逐分量绝对值最大值，但本项目不直接修改 `../warpx`；在重新运行并决定阈值前，该 case 仍只能写成“官方 gate 通过、强化 absolute gate 失败”的判据边界。源码报告归档于 `runs/stage-c-validation/particles-in-pml-analysis-source-contract/`，详细说明见 `notes/code-reading/particles/51-particles-in-pml-signed-vs-absolute-contract.md`。
