...
<!-- source: manuscript/VERSION-v0.7.md -->
...
`v0.7` 是 `PIC-tutor` 的场求解器验证入口表版。它继承 `v0.6` 的第一卷范围，并把第 6 章 `电磁场求解器` 从“读者侧分派图和对照表”继续推进到“能从正文直接回到 WarpX regression 入口和 analysis 判据”的状态。

本版的重点仍不是扩写新章节，而是把第 6 章 v0.5/v0.6 已校准的源码证据继续和测试证据闭合：新增具体 regression 入口索引表，覆盖 `Examples/Tests/pml`、`nci_psatd_stability`、`langmuir` 和 RZ PSATD/JRhom 相关 tests。表中明确列出代表输入、analysis 脚本、主要判据和对应源码风险，避免把 checksum 或 restart smoke 误写成物理强判据。

本版仍不是出版终稿。它的目标是把第 2、3、3A、4、5、6 章先变成能按当前源码逐段复查、能被读者较顺畅进入、并能回到 regression 判据的骨架；下一批需要继续收束第 6 章的文献闭环，以及第 7 章边界、PML 与 AMR。
...
v0.7 只修改 `PIC-tutor` 书稿项目，不修改 `../warpx` 原仓库。章节里的源码行号按上述 checkout 复核；后续若 WarpX 更新，必须重新校准行号后再发布新版。
...
- 冻结 `manuscript/VERSION-v0.6.md`，避免重建 v0.6 时误用 v0.7 版本说明。
- 新增 v0.7 当前版本说明，并把构建输出切到 `dist/pic-tutor-v0.7.md` 与 `dist/pic-tutor-v0.7.html`。
- 继续把第 6 章绑定到当前 WarpX commit `8c488b1a9`，但本版主要收束 regression 证据，不重新扩大源码范围。
- 在第 6 章 `FieldSolver regression 判据` 中新增具体 regression 入口索引表。
- 表格覆盖 PML FDTD、PML PSATD/Galilean、PML restart、RZ PML PSATD、Galilean PSATD NCI、averaged Galilean PSATD、PSATD-JRhom NCI、RZ PSATD-JRhom smoke、Langmuir 2D/3D/RZ 等路径。
- 明确每条路径的代表输入、analysis 脚本、主要判据和对应源码风险，区分物理强判据、checksum 和 restart smoke。
- 把 `scripts/build_v06.py` 改为读取冻结的 `manuscript/VERSION-v0.6.md`，保持 v0.6 版本说明可复现。
...
- 建立稳定的 Markdown/HTML/PDF 构建流程；v0.7 仍只保证 Markdown/HTML。
...
```bash
python scripts/build_v07.py
```

生成的文件：

- `dist/pic-tutor-v0.7.md`
- `dist/pic-tutor-v0.7.html`（若本机存在 `pandoc`）
