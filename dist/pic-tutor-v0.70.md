...
本版延续第 9 章的 `docs/literature-pending-triage.md`：对 literature map 中仍待分类的 83 条 BibTeX 条目保留保守的候选主题、章节入口、P1/P2/P3 优先级和获取动作。该 triage 只服务于 acquisition/read queue，不把标题关键词、metadata 或建议章节升级为全文阅读或正文证据。

本版由 `scripts/verify_v70_build.py` 验收，当前合订 PDF 页数以构建结果为准。
