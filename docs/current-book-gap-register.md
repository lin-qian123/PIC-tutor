...
本表是 v0.84 的当前状态快照，不是历史 TODO 的完整替代品。每一项都必须同时有：当前证据、明确分类、下一步动作和可验收的关闭条件。`OPEN_EXTERNAL_ACCESS`、`RUNTIME_LEDGER_UNPROVEN`、`PRE_PHYSICS_BOUNDARY` 和 `UNPROVEN` 都表示尚未完成，不是 PASS。
...
| `STUDY-FORMAL-CONVERGENCE` | 跨 resolution/observable 的正式收敛阶 | `FORMAL_CONVERGENCE_PREREGISTERED_CURRENT_DATA_INSUFFICIENT` | `docs/formal-convergence-preregistration.json`、`runs/stage-c-validation/formal-convergence-preregistration/contract.json`、`runs/stage-c-validation/cross-geometry-convergence-trends/contract.json` | 按预注册固定 norm、控制变量和拟合区间，补每种 geometry 的第二组独立 family；当前 slope 仍分开、不做 pooled fit | 两组独立 family 的 order 在预注册 gate 内稳定，且 charge boundary 有独立证据 |
