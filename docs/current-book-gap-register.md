...
本表是 v0.83 的当前状态快照，不是历史 TODO 的完整替代品。每一项都必须同时有：当前证据、明确分类、下一步动作和可验收的关闭条件。`OPEN_EXTERNAL_ACCESS`、`RUNTIME_LEDGER_UNPROVEN`、`PRE_PHYSICS_BOUNDARY` 和 `UNPROVEN` 都表示尚未完成，不是 PASS。
...
| `STUDY-FORMAL-CONVERGENCE` | 跨 resolution/observable 的正式收敛阶 | `EXPLORATORY_CROSS_GEOMETRY_RESOLUTION_TRENDS_FORMAL_ORDER_UNPROVEN` | `runs/stage-c-validation/deposition-convergence-readiness/contract.json`、`runs/stage-c-validation/cross-geometry-convergence-trends/contract.json` | 继续预注册 norm、控制变量、拟合区间和重复 family；当前 RZ/RSPHERE slope 分开，不做 pooled fit | 重复/独立 family 的 order 在预注册 gate 内稳定 |
