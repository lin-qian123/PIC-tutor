...
- Phase 2 executed on 2026-08-14: `03-warpx-evolve`, `03a-warpx-initialization`, and appendix A migrated to native `.tex`; book now 115 pages (preface + ch1--3a + appendix A); per-chapter automated gates PASS. Report: `docs/latex-phase2-migration-report-v0.110.md`. Converter fixes recorded there (list-state machine bug, single-line `$$…$$` in table cells, appendix title prefix, enumitem depth). Next: Phase 3 (04--09).
- Phase 3 executed on 2026-08-14: `04-particle-pushers` through `09-literature-roadmap` migrated to native `.tex`; **full book (all 13 source parts) now compiles at 404 pages**; automated gate PASS (14 figures found, 0 missing chars/refs/figures, overfull 12 all ≤ 57 pt within policy). Report: `docs/latex-phase3-migration-report-v0.110.md`. Converter fixes recorded there (code-scan runaway from `%` in `\path`/`\code`, `\codeesc` escaping, math-aware table cell splitting, seqsplit breaking, slash-chain `\allowbreak`). Next: Phase 4 full-book closure and edition release.
- Phase 4 executed on 2026-08-14: three-theme builds (academic 404pp / technical 408pp / compact 373pp) all PASS automated gates; **reproducible byte-identical clean rebuilds** (`SOURCE_DATE_EPOCH=0`); provisional edition identifier **`v0.120`** with cumulative manifest (`dist/latex/manifest.json`); migration report `docs/latex-phase4-migration-report-v0.110.md`. **Final edition numbering, release, and the Phase 1 decision point remain pending maintainer approval** (no GitHub Release created; `v1.0` tag untouched; no license chosen).
...
3. ~~Phase 2 foundation/lifecycle chapters (00-preface, 01, 02, 03, 03a, appendix A).~~ Done 2026-08-14 (03/03a/A added this round; evidence `docs/latex-phase2-migration-report-v0.110.md`); 115-page book, per-chapter gates PASS.
4. ~~Phase 3 algorithm/evidence chapters (04--09).~~ Done 2026-08-14; full book (all 13 source parts) compiles at 404 pages, gate PASS (evidence `docs/latex-phase3-migration-report-v0.110.md`).
5. Compare the sample against the current `academic` theme PDF and decide whether the native-LaTeX result justifies full migration. **Pending: maintainer approves the Phase 1 decision point (document class, fonts, themes, code/table/float strategy).**
6. ~~Phase 4 closure: three-theme builds, reproducible double build, new edition identifier + release manifest, docs updates, migration report, page-level visual review.~~ Done 2026-08-14 (technical work complete; evidence `docs/latex-phase4-migration-report-v0.110.md`). Remaining: maintainer page-level visual review and release decision (edition numbering, GitHub Release, license, redistribution).
