...
4. Compare the sample against the current `academic` theme PDF and decide whether the native-LaTeX result justifies full migration. **Pending: maintainer approves the Phase 1 decision point (document class, fonts, themes, code/table/float strategy).**
5. After explicit sample approval, continue the chapter-by-chapter ownership cutover in Phase 3 (04--09) and Phase 4 closure.
