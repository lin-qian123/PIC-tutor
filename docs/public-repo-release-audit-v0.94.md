...
当前 release 为 v0.94；本版在 v0.92 两组独立 2-rank family 基础上完成 correction-on axis charge repeat stability contract：6 个 correction-on level 全部通过 `1e-10` 相对重复容差，且 axis residual 持续高于 off-axis。当前 kernel root-cause、正式 order closure、人工全书通读、许可和公开再分发仍保持开放。
...
当前 `dist/pic-tutor-v0.94.pdf` 页数以本轮构建验收为准；v0.75 及此前的构建记录均属于历史构建快照。
...
| `dist/pic-tutor-v0.94.pdf` | 3,477,482 bytes / 331 页 | 当前成书候选，可单独审计后发布 |
| `dist/pic-tutor-v0.94.html` | 5,851,988 bytes | 自包含 MathJax + 16 张图片，可作为预览候选 |
| `dist/pic-tutor-v0.94.md` | 1,164,044 bytes | 当前合订源，可作为文本 release 候选 |

## 当前边界

- `runs/` 已加入忽略规则；本轮又忽略了根目录运行残留 `Backtrace.*` 和 `bmmntr.txt`。
- 当前书稿图表位于 `manuscript/assets/figures/`，源章节使用相对路径；`scripts/build_v94.py` 会在合订阶段解析资源。
- `dist/` 当前仍保留历史版本产物，发布时应明确选择 `v0.94`，不要按目录整体上传。
...
- `scripts/build_v94.py`、`scripts/verify_v94_build.py`、`scripts/audit_release_consistency.py`、`scripts/run_formal_convergence_repeat_family.py`、`scripts/analyze_rz_axis_charge_repeat_stability.py` 及书稿引用的项目分析脚本；
...
- `dist/pic-tutor-v0.94.md`、`dist/pic-tutor-v0.94.html`、`dist/pic-tutor-v0.94.pdf`，前提是维护者确认生成物的发布策略。
...
```bash
python scripts/build_v94.py
python scripts/verify_v94_build.py --build-log /tmp/pic-tutor-build-v94.log
python scripts/audit_release_consistency.py
git status --short
```

当前 v0.94 构建验收结果以 `scripts/verify_v94_build.py`、`scripts/audit_editorial_quality.py`、`scripts/audit_pdf_layout.py`、`scripts/audit_formal_convergence_preregistration.py`、`scripts/run_formal_convergence_repeat_family.py`、`scripts/analyze_formal_convergence_repeat_family.py` 和 `scripts/analyze_rz_axis_charge_repeat_stability.py` 为准；kernel root-cause、正式收敛阶、人工全书通读、许可和公开再分发仍保持开放。

v0.94 发布 allowlist 另见 `docs/v0.94-release-manifest.{json,md}`；总字节数以 manifest 的 `total_bytes` 为准，`runs/`、`references/`、历史 `dist/` 和调试残留均被排除。该 manifest 是审计输入，不自动执行 Git staging、commit 或 push。
