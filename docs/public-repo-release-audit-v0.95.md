...
当前 `dist/pic-tutor-v0.95.pdf` 页数以本轮构建验收为准；v0.75 及此前的构建记录均属于历史构建快照。
...
- 当前书稿图表位于 `manuscript/assets/figures/`，源章节使用相对路径；`scripts/build_v95.py` 会在合订阶段解析资源。
- `dist/` 当前仍保留历史版本产物，发布时应明确选择 `v0.95`，不要按目录整体上传。
...
- `dist/pic-tutor-v0.95.md`、`dist/pic-tutor-v0.95.html`、`dist/pic-tutor-v0.95.pdf`，前提是维护者确认生成物的发布策略。
