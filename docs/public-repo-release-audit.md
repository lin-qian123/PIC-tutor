...
| `dist/pic-tutor-v0.68.pdf` | 3,360,324 bytes | 当前 316 页成书候选，可单独审计后发布 |
| `dist/pic-tutor-v0.68.html` | 5,736,169 bytes | 自包含 MathJax + 16 张图片，可作为预览候选 |
| `dist/pic-tutor-v0.68.md` | 1,064,904 bytes | 当前合订源，可作为文本 release 候选 |
...
v0.68 发布 allowlist 另见 `docs/v0.68-release-manifest.{json,md}`：当前包含 695 个项目文件、总计 `27,400,477` bytes；`runs/`、`references/`、历史 `dist/` 和调试残留均被排除。该 manifest 是审计输入，不自动执行 Git staging、commit 或 push。
