...
| `dist/pic-tutor-v0.68.pdf` | 3,392,111 bytes | 当前 321 页成书候选，可单独审计后发布 |
...
v0.68 发布 allowlist 另见 `docs/v0.68-release-manifest.{json,md}`：当前包含 748 个项目文件、总计 `27,818,700` bytes；`runs/`、`references/`、历史 `dist/` 和调试残留均被排除。该 manifest 是审计输入，不自动执行 Git staging、commit 或 push。
