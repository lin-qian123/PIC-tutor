...
当前 release 为 v0.107；本版将非中性 RZ shape family 扩展到 `64x128` 与 `128x256`，species axis 比值跨分辨率稳定，但高分辨率 shape=2/3/4 的 total-rho 因 species cancellation 变为 `1/1/1`，最大差为 `0.168327256`。分类为 `RZ_NONNEUTRAL_AXIS_CORRECTION_SHAPE_DEPENDENT_CROSS_RESOLUTION_BOUNDARY_OPEN`，不把该结果写成 root cause、charge closure 或正式 order closure。route-count ledger、axis charge correctness、RZ implicit runtime、人工全书通读、许可和公开再分发仍保持开放。
...
当前 `dist/pic-tutor-v0.107.pdf` 页数以本轮构建验收为准；v0.75 及此前的构建记录均属于历史构建快照。
...
| `dist/pic-tutor-v0.107.pdf` | 3,510,512 bytes / 337 页 | 当前成书候选，可单独审计后发布 |
| `dist/pic-tutor-v0.107.html` | 5,895,478 bytes | 自包含 MathJax + 16 张图片，可作为预览候选 |
| `dist/pic-tutor-v0.107.md` | 1,192,905 bytes | 当前合订源，可作为文本 release 候选 |
...
- `dist/` 当前仍保留历史版本产物，发布时应明确选择 `v0.107`，不要按目录整体上传。
...
- `dist/pic-tutor-v0.107.md`、`dist/pic-tutor-v0.107.html`、`dist/pic-tutor-v0.107.pdf`，前提是维护者确认生成物的发布策略。
...
```bash
python scripts/build_v107.py
python scripts/verify_v107_build.py --build-log /tmp/pic-tutor-build-v107.log
python scripts/audit_release_consistency.py
git status --short
```

当前 v0.107 构建验收结果以 `scripts/verify_v107_build.py`、`scripts/audit_rz_axis_correction_nonneutral_control.py`、`scripts/audit_rz_axis_correction_nonneutral_shape_family.py`、`scripts/audit_rz_axis_correction_nonneutral_resolution_family.py`、`scripts/audit_editorial_quality.py`、`scripts/audit_pdf_layout.py`、`scripts/audit_formal_convergence_preregistration.py`、`scripts/audit_formal_convergence_repeat_slope_gate.py`、`scripts/run_formal_convergence_repeat_family.py`、`scripts/analyze_formal_convergence_repeat_family.py`、`scripts/analyze_rz_axis_charge_repeat_stability.py`、`scripts/analyze_rz_axis_divergence_stencil_contract.py`、`scripts/analyze_rz_axis_divergence_resolution_contract.py`、`scripts/analyze_rz_axis_divergence_fit_contract.py` 和 `scripts/audit_transition_zone_runtime_activation.py` 为准；route-count ledger、axis charge correctness、正式收敛阶、人工全书通读、许可和公开再分发仍保持开放。

v0.107 发布 allowlist 另见 `docs/v0.107-release-manifest.{json,md}`；总字节数以 manifest 的 `total_bytes` 为准，`runs/`、`references/`、历史 `dist/` 和调试残留均被排除。该 manifest 是审计输入，不自动执行 Git staging、commit 或 push。
