...
当前 release 为 v0.96；本版在 v0.95 repeat-slope gate 基础上补入 Esirkepov 2001 与 LeeCPC2015 的 DOI/PII、publisher record 和 PDF 路由访问边界复核。当前 axis charge correctness、正式 order closure、transition-zone ledger、RZ implicit runtime、人工全书通读、许可和公开再分发仍保持开放。
...
当前 `dist/pic-tutor-v0.96.pdf` 页数以本轮构建验收为准；v0.75 及此前的构建记录均属于历史构建快照。
...
| `dist/pic-tutor-v0.96.pdf` | 3,484,239 bytes / 333 页 | 当前成书候选，可单独审计后发布 |
| `dist/pic-tutor-v0.96.html` | 5,857,745 bytes | 自包含 MathJax + 16 张图片，可作为预览候选 |
| `dist/pic-tutor-v0.96.md` | 1,168,853 bytes | 当前合订源，可作为文本 release 候选 |
...
- `dist/` 当前仍保留历史版本产物，发布时应明确选择 `v0.96`，不要按目录整体上传。
...
- `dist/pic-tutor-v0.96.md`、`dist/pic-tutor-v0.96.html`、`dist/pic-tutor-v0.96.pdf`，前提是维护者确认生成物的发布策略。
...
当前 v0.96 构建验收结果以 `scripts/verify_v96_build.py`、`scripts/audit_editorial_quality.py`、`scripts/audit_pdf_layout.py`、`scripts/audit_formal_convergence_preregistration.py`、`scripts/audit_formal_convergence_repeat_slope_gate.py`、`scripts/run_formal_convergence_repeat_family.py`、`scripts/analyze_formal_convergence_repeat_family.py` 和 `scripts/analyze_rz_axis_charge_repeat_stability.py` 为准；axis charge correctness、正式收敛阶、人工全书通读、许可和公开再分发仍保持开放。

v0.96 发布 allowlist 另见 `docs/v0.96-release-manifest.{json,md}`；总字节数以 manifest 的 `total_bytes` 为准，`runs/`、`references/`、历史 `dist/` 和调试残留均被排除。该 manifest 是审计输入，不自动执行 Git staging、commit 或 push。
