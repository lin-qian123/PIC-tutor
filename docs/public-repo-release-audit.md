...
当前 `dist/pic-tutor-v0.68.pdf` 为 315 页，内含 16 张书稿验证图；v0.67 及此前的构建记录均属于历史构建快照。
...
| `dist/pic-tutor-v0.68.pdf` | 3,354,894 bytes | 当前 315 页成书候选，可单独审计后发布 |
| `dist/pic-tutor-v0.68.html` | 5,732,044 bytes | 自包含 MathJax + 16 张图片，可作为预览候选 |
| `dist/pic-tutor-v0.68.md` | 1,061,674 bytes | 当前合订源，可作为文本 release 候选 |
...
当前 v0.68 构建验收结果：`pdf_pages=315`、源/合订图片链接均为 `16`、HTML 内嵌图片和图号范围检查全部通过。

v0.68 发布 allowlist 另见 `docs/v0.68-release-manifest.{json,md}`：当前包含 694 个项目文件、总计 `27,376,601` bytes；`runs/`、`references/`、历史 `dist/` 和调试残留均被排除。该 manifest 是审计输入，不自动执行 Git staging、commit 或 push。
