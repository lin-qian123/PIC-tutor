...
当前 release 为 v0.87；本版修复 v0.86 第 5 章新增收敛执行页的长 token PDF 截断，并保留正式收敛第二组 family 的 2-rank runner 输入与产物合同。当前 MPI launcher、第二组独立 family、正式收敛阶、人工全书通读、许可和公开再分发仍保持开放。
...
当前 `dist/pic-tutor-v0.87.pdf` 页数以本轮构建验收为准；v0.75 及此前的构建记录均属于历史构建快照。
...
| `dist/pic-tutor-v0.87.pdf` | 3,467,091 bytes / 330 页 | 当前成书候选，可单独审计后发布 |
| `dist/pic-tutor-v0.87.html` | 5,840,723 bytes | 自包含 MathJax + 16 张图片，可作为预览候选 |
| `dist/pic-tutor-v0.87.md` | 1,154,312 bytes | 当前合订源，可作为文本 release 候选 |

## 当前边界

- `runs/` 已加入忽略规则；本轮又忽略了根目录运行残留 `Backtrace.*` 和 `bmmntr.txt`。
- 当前书稿图表位于 `manuscript/assets/figures/`，源章节使用相对路径；`scripts/build_v87.py` 会在合订阶段解析资源。
- `dist/` 当前仍保留历史版本产物，发布时应明确选择 `v0.87`，不要按目录整体上传。
...
- `scripts/build_v87.py`、`scripts/verify_v87_build.py`、`scripts/audit_release_consistency.py`、`scripts/run_formal_convergence_repeat_family.py` 及书稿引用的项目分析脚本；
...
- `dist/pic-tutor-v0.87.md`、`dist/pic-tutor-v0.87.html`、`dist/pic-tutor-v0.87.pdf`，前提是维护者确认生成物的发布策略。
...
```bash
python scripts/build_v87.py
python scripts/verify_v87_build.py --build-log /tmp/pic-tutor-build-v87-final.log
python scripts/audit_release_consistency.py
git status --short
```

当前 v0.87 构建验收结果以 `scripts/verify_v87_build.py`、`scripts/audit_editorial_quality.py`、`scripts/audit_pdf_layout.py`、`scripts/audit_formal_convergence_preregistration.py` 和 `scripts/run_formal_convergence_repeat_family.py` 为准；MPI launcher、第二组独立 family、正式收敛阶、人工全书通读、许可和公开再分发仍保持开放。

v0.87 发布 allowlist 另见 `docs/v0.87-release-manifest.{json,md}`；总字节数以 manifest 的 `total_bytes` 为准，`runs/`、`references/`、历史 `dist/` 和调试残留均被排除。该 manifest 是审计输入，不自动执行 Git staging、commit 或 push。
