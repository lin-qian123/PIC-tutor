...
v0.68 发布 allowlist 另见 `docs/v0.68-release-manifest.{json,md}`：当前包含 729 个项目文件、总计 `27,639,472` bytes；`runs/`、`references/`、历史 `dist/` 和调试残留均被排除。该 manifest 是审计输入，不自动执行 Git staging、commit 或 push。
