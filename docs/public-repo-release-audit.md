...
当前 release 为 v0.99；本版在 v0.98 axis divergence stencil alignment 基础上，将独立 reader 对照扩展到三档 correction-on/off resolution family，6/6 case 均支持源码 `4*Er/dr`。当前 route-count ledger、axis charge correctness、正式 order closure、RZ implicit runtime、人工全书通读、许可和公开再分发仍保持开放。
...
当前 `dist/pic-tutor-v0.99.pdf` 页数以本轮构建验收为准；v0.75 及此前的构建记录均属于历史构建快照。
...
| `dist/pic-tutor-v0.99.pdf` | 3,490,203 bytes / 334 页 | 当前成书候选，可单独审计后发布 |
| `dist/pic-tutor-v0.99.html` | 5,865,705 bytes | 自包含 MathJax + 16 张图片，可作为预览候选 |
| `dist/pic-tutor-v0.99.md` | 1,174,161 bytes | 当前合订源，可作为文本 release 候选 |
...
- `dist/` 当前仍保留历史版本产物，发布时应明确选择 `v0.99`，不要按目录整体上传。
...
- `scripts/build_v95.py`、`scripts/verify_v95_build.py`、`scripts/audit_release_consistency.py`、`scripts/run_formal_convergence_repeat_family.py`、`scripts/analyze_rz_axis_charge_repeat_stability.py` 及书稿引用的项目分析脚本；
...
- `dist/pic-tutor-v0.99.md`、`dist/pic-tutor-v0.99.html`、`dist/pic-tutor-v0.99.pdf`，前提是维护者确认生成物的发布策略。
...
```bash
python scripts/build_v95.py
python scripts/verify_v95_build.py --build-log /tmp/pic-tutor-build-v95.log
python scripts/audit_release_consistency.py
git status --short
```

当前 v0.99 构建验收结果以 `scripts/verify_v99_build.py`、`scripts/audit_editorial_quality.py`、`scripts/audit_pdf_layout.py`、`scripts/audit_formal_convergence_preregistration.py`、`scripts/audit_formal_convergence_repeat_slope_gate.py`、`scripts/run_formal_convergence_repeat_family.py`、`scripts/analyze_formal_convergence_repeat_family.py`、`scripts/analyze_rz_axis_charge_repeat_stability.py`、`scripts/analyze_rz_axis_divergence_stencil_contract.py`、`scripts/analyze_rz_axis_divergence_resolution_contract.py` 和 `scripts/audit_transition_zone_runtime_activation.py` 为准；route-count ledger、axis charge correctness、正式收敛阶、人工全书通读、许可和公开再分发仍保持开放。

v0.99 发布 allowlist 另见 `docs/v0.99-release-manifest.{json,md}`；总字节数以 manifest 的 `total_bytes` 为准，`runs/`、`references/`、历史 `dist/` 和调试残留均被排除。该 manifest 是审计输入，不自动执行 Git staging、commit 或 push。
