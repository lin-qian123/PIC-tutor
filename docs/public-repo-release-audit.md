...
当前 `dist/pic-tutor-v0.69.pdf` 为 326 页，内含 16 张书稿验证图；v0.68 及此前的构建记录均属于历史构建快照。

v0.66 审计发现合订 Markdown/HTML 含本机绝对路径和不可随公共仓库迁移的绝对链接；v0.67 已在 release 构建层修复，v0.69 延续该检查，并将验证合同摘要与文献 triage 纳入公共 allowlist。

v0.69 延续 `docs/public-evidence-index.{json,md}`，从本地 187 条 `contract.json` 生成去路径摘要（`PASS 135 / FAIL 30 / UNKNOWN 22`），并加入 `docs/literature-pending-triage.md` 的 acquisition/read queue。摘要保留原始合同状态，并将 boundary、unproven、missing 分类单独标记；原始 `runs/` 仍排除，因此该摘要是公共证据目录，不是运行产物替代品。
...
| `dist/pic-tutor-v0.69.pdf` | 3,420,579 bytes | 当前 326 页成书候选，可单独审计后发布 |
| `dist/pic-tutor-v0.69.html` | 5,794,552 bytes | 自包含 MathJax + 16 张图片，可作为预览候选 |
| `dist/pic-tutor-v0.69.md` | 1,113,415 bytes | 当前合订源，可作为文本 release 候选 |
...
- `dist/` 当前仍保留历史版本产物，发布时应明确选择 `v0.69`，不要按目录整体上传。
...
- `dist/pic-tutor-v0.69.md`、`dist/pic-tutor-v0.69.html`、`dist/pic-tutor-v0.69.pdf`，前提是维护者确认生成物的发布策略。
...
当前 v0.69 构建验收结果：`pdf_pages=326`、源/合订图片链接均为 `16`、HTML 内嵌图片和图号范围检查全部通过。

v0.69 发布 allowlist 另见 `docs/v0.69-release-manifest.{json,md}`：当前包含 786 个项目文件；总字节数以 manifest 的 `total_bytes` 为准，`runs/`、`references/`、历史 `dist/` 和调试残留均被排除。该 manifest 是审计输入，不自动执行 Git staging、commit 或 push。
