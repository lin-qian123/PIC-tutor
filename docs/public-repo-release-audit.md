...
v0.68 新增 `docs/public-evidence-index.{json,md}`，从本地 159 条 `contract.json` 生成去路径摘要。摘要保留原始合同状态，并将 boundary、unproven、missing 分类单独标记；原始 `runs/` 仍排除，因此该摘要是公共证据目录，不是运行产物替代品。
...
| `dist/pic-tutor-v0.68.pdf` | 3,380,678 bytes | 当前 319 页成书候选，可单独审计后发布 |
| `dist/pic-tutor-v0.68.html` | 5,754,980 bytes | 自包含 MathJax + 16 张图片，可作为预览候选 |
| `dist/pic-tutor-v0.68.md` | 1,081,206 bytes | 当前合订源，可作为文本 release 候选 |
...
v0.68 发布 allowlist 另见 `docs/v0.68-release-manifest.{json,md}`：当前包含 734 个项目文件、总计 `27,681,476` bytes；`runs/`、`references/`、历史 `dist/` 和调试残留均被排除。该 manifest 是审计输入，不自动执行 Git staging、commit 或 push。
