...
本版重新执行正式收敛 study 的第二组独立 family：RZ/RSPHERE 各包含 `64/128/256`、correction on/off，共 12 个真实 2-rank producer；全部返回 0 并生成 diagnostics。使用相同 reader-side norm 的 correction-on repeat-slope gate 共 14 项，全部通过 `1e-8` tolerance，最大绝对 slope 差为 `2.0135e-11`；correction-off 最大差为 `1.736e-3`，继续只作 numerical/reader-floor negative control。分类为 `FORMAL_CONVERGENCE_REPEAT_SLOPE_GATE_PASS_CHARGE_CLOSURE_OPEN`，只确认 gate 可复现，不关闭 formal numerical order 或 axis-charge correctness。报告见 `runs/stage-c-validation/formal-convergence-second-family-v0.110/contract.{json,md}`、`runs/stage-c-validation/formal-convergence-repeat-slope-gate-v0.110/contract.{json,md}`，说明见 `notes/code-reading/particles/91-formal-convergence-repeat-slope-gate-v0.110.md`。
...
本版由 `scripts/verify_v110_build.py` 验收，当前合订 PDF 页数以实际构建结果为准。
