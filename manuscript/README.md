...
当前 v0.109 合订 PDF 的页数以实际构建结果为准；本版新增第三种离子密度的 `128x256` 非中性 RZ shape family，证明 `0.25*n0` 与 `0.75*n0` 的 total-rho 序列一致，而 `0.5*n0` 的 shape=2/3/4 是 sampled-axis cancellation。正文继续区分“species 证据”与“root cause/charge closure 已完成”，不把 reader 对照升级为完整物理通过。
