...
当前 v0.52 合订 PDF 为 310 页；页数、图表资源、关键标记和构建警告均由 `scripts/verify_v52_build.py` 验收。第 1-8 章均已补入至少一个可执行的练习、源码定位题或复现实验任务。
...
同日又完成 RSPHERE 64/128 resolution 与 axis correction paired control；field 全通过，charge 保持为 resolution-sensitive BOUNDARY，不宣称收敛阶。
