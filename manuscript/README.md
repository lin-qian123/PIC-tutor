...
当前 v0.79 合订 PDF 的页数以实际构建结果为准；本版新增第 5 章收敛研究就绪合同，并保留沉积证据梯度、第 7 章 PML 证据梯度与 Esirkepov 2001 CPC bounded compare。
...
当前 v0.79 合订 PDF 页数以实际构建结果为准；页数、图表资源、关键标记、public path hygiene 和构建警告均由 `scripts/verify_v79_build.py` 验收。第 1-9 章均已补入至少一个可执行的练习、源码定位题或复现实验任务。
