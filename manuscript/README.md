...
这是 `PIC-tutor` 的 Markdown-first 书稿。当前收束版本是 `v0.3` 粒子推进器校准草稿，目标是先形成一条可复查的主线，再逐章加深公式推导、源码逐行讲解、图表和可复现实验。
...
- 第 2、3、3A、4 章已按当前 checkout 重新核对核心源码行号；其他章节中出现的旧 commit 表示当时阅读基线，后续仍需逐章校准。
- 本书稿不修改 WarpX 原仓库。
