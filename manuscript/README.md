...
当前 v0.82 合订 PDF 的页数以实际构建结果为准；本版修复第 7 章边界 regression 宽表的 PDF 列重叠，并新增页面布局风险审计；同时保留项目级当前成书缺口登记、第 5 章收敛研究就绪合同、沉积证据梯度、第 7 章 PML 证据梯度与 Esirkepov 2001 CPC bounded compare。
...
当前 v0.82 合订 PDF 页数以实际构建结果为准；页数、图表资源、关键标记、public path hygiene 和构建警告均由 `scripts/verify_v82_build.py` 验收，页面布局风险由 `scripts/audit_pdf_layout.py` 复核。第 1-9 章均已补入至少一个可执行的练习、源码定位题或复现实验任务。
