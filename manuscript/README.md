...
这是 `PIC-tutor` 的 Markdown-first 书稿。当前收束版本是 `v0.9` 边界 regression 判据索引草稿，目标是先形成一条可复查的主线，再逐章加深公式推导、源码逐行讲解、图表和可复现实验。
...
- 第 2、3、3A、4、5、6 章已按当前 checkout 重新核对核心源码行号；第 7 章已完成 v0.9 regression 判据索引，后续仍需继续细核 AMR/regrid/guard-cell 并把索引表扩写成正文。
- 本书稿不修改 WarpX 原仓库。
