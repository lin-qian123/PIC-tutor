...
当前 v0.73 合订 PDF 的页数以实际构建结果为准；本版新增 Andriyash 2016 Fourier-Bessel PIC 全文资产，补入第 6 章 RZ/准柱坐标 PSATD 的 primary-source 证据。
...
当前 v0.73 合订 PDF 页数以实际构建结果为准；页数、图表资源、关键标记、public path hygiene 和构建警告均由 `scripts/verify_v73_build.py` 验收。第 1-9 章均已补入至少一个可执行的练习、源码定位题或复现实验任务。
