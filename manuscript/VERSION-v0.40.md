...
当前合订 PDF 为 307 页；第 1/2/3/4/5/6 章的新增内容、第 2/3/3A 章读者侧流程图、第 1-8 章练习入口、完整 `initial_distribution` 官方分布 analysis、native Gaussian external-file 验证、RZ charge/inverse-volume 验证、RZ 三模态 writeback 验证、Cartesian/RZ PSATD-PML 验证、RZ JRhom first-stage contract、uniform-plasma 3D 2-rank restart、particle boundary families、RZ/3D/2D EB flux-injection families、PEC/PMC/PML field-boundary families 以及 MPI consistency 边界图已纳入本版构建和页数验收基线。
