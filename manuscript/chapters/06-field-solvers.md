...
因此，这一小块现在的闭合条件已经可以写清楚：第一，正文诚实记录当前 `analysis=OFF`、第一阶段 finite/energy/spike gate 和第二阶段 `divE` gate 之间的证据等级差异；第二，计划笔记已经补出 stable ledger、unstable contrast、provenance note 和 patch 四件套清单；第三，下一步要么真的 materialize 一份 comoving reference ledger，要么转去做 RZ PSATD validation 强判据表，而不是继续只写一层抽象方案。达到这一步之前，没有必要再为它单独切一个新版本号。
