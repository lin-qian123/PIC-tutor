#!/usr/bin/env python
"""Independent reader-side contract for the 3D EB flux-injection test."""
...
def main() -> int:
    ...
    y = data["electron", "particle_position_y"].to_ndarray()
    z = data["electron", "particle_position_z"].to_ndarray()
    ...
    radius = 2.0
    expected_weight = 1.0 * (4.0 * np.pi * radius**2) * 0.5e-8
    radial = np.sqrt(x**2 + y**2 + z**2)
    nx, ny, nz = x / radial, y / radial, z / radial
    normal = ux * nx + uy * ny + uz * nz
    denom = np.sqrt(nx**2 + ny**2)
    tx, ty, tz = ny / denom, -nx / denom, np.zeros_like(nx)
    perpendicular = ux * tx + uy * ty + uz * tz
    sx, sy, sz = -nz * ty, nz * tx, nx * ty - ny * tx
    perpendicular2 = ux * sx + uy * sy + uz * sz
    ...
    result = {
        "case": "test_3d_flux_injection_from_eb",
