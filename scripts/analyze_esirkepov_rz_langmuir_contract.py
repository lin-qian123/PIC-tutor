#!/usr/bin/env python
"""Independent reader-side contract for the RZ Esirkepov Langmuir case."""
...
def main() -> None:
    ...
    er_theory = epsilon * m_e * c**2 / e * 2.0 * rr / w0**2 * np.exp(-(rr**2) / w0**2) * np.sin(k0 * zz) * np.sin(wp * t)
    ez_theory = -epsilon * m_e * c**2 / e * k0 * np.exp(-(rr**2) / w0**2) * np.cos(k0 * zz) * np.sin(wp * t)
    ...
    result = {
        "contract": "Esirkepov RZ Langmuir reader-side runtime contract",
        ...
        "scope": "independent RZ Er/Ez and same-surface divE-rho/epsilon0 check; not a full geometry/order matrix",
    }
    args.output_json.parent.mkdir(parents=True, exist_ok=True)
    args.output_json.write_text(json.dumps(result, indent=2) + "\n", encoding="utf-8")
    args.output_md.parent.mkdir(parents=True, exist_ok=True)
    args.output_md.write_text(
        "# Esirkepov RZ Langmuir runtime contract\n\n"
        ...
        f"- axis-cell charge residual: `{axis_charge_relative:.8e}`\n"
        f"- off-axis charge residual (r index >= 1): `{off_axis_charge_relative:.8e}`\n"
    ...
    print(
        f"{'PASS' if result['passed'] else 'BOUNDARY'}: "
        f"Er/Ez={er_error:.3e}/{ez_error:.3e}, charge residual={charge_relative:.3e}"
    )
    if not field_passed:
        raise SystemExit("RZ Langmuir field contract failed")
