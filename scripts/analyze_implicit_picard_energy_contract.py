#!/usr/bin/env python
"""Analyze the reduced total-energy contract for a 1D implicit Picard run."""
...
def main() -> None:
    ...
    parser.add_argument("--scheme", required=True, choices=("semi-implicit", "theta-implicit"))
    ...
    result = {
        ...
        "passed": max_abs_delta < args.tolerance,
        "contract": f"{args.scheme} Picard total particle-plus-field energy bound",
    ...
    args.output_md.write_text(
        f"# {args.scheme.title()} Picard energy contract\n\n"
        f"- status: `{status}`\n"
    ...
    if not result["passed"]:
        raise SystemExit("implicit Picard energy contract failed")
