#!/usr/bin/env python
"""Independent field contract for the RZ Langmuir PSATD-JRhom sibling."""
...
def main() -> None:
    ...
    jrhom = re.search(r'^psatd\.JRhom\s*=\s*["\']?(\S+?)["\']?\s*$', inputs, re.MULTILINE)
    ...
    if not jrhom or jrhom.group(1) != "CL4":
        raise AssertionError("expected psatd.JRhom = CL4")
    ...
    result = {
        "contract": "RZ PSATD-JRhom CL4 Langmuir field contract",
    ...
    args.output_md.write_text(
        "# RZ PSATD-JRhom CL4 Langmuir contract\n\n"
        f"- JRhom: `{jrhom.group(1)}`\n"
    ...
    if not field_passed:
        raise SystemExit("RZ PSATD-JRhom field contract failed")
