...
def main() -> int:
    ...
    sources = {
        ...
        "pec_insulator": read(warpx / "Source/BoundaryConditions/PEC_Insulator.H"),
    ...
    checks = [
        ...
        ("chapter_pml", "正文 PML split-field 与电流路径", chapter, ["split fields", "DampPML()", "Exchange()", "pml_has_particles"]),
        ...
        ("chapter_window_scrape", "正文 moving window 与 scraping diagnostics", chapter, ["moving window", "BoundaryScrapingDiagnostics", "scraping buffer"]),
        ("chapter_transition_boundary", "正文 transition-zone 未完成边界", chapter, ["transition zone", "route ledger", "没有 dedicated route-count regression"]),
        ("source_boundary_order", "WarpX 构造期边界解析", sources["warpx"], ["parse_field_boundaries", "get_periodicity_array", "parse_particle_boundaries"]),
        ("source_field_dispatch", "场边界运行时分派", sources["field_boundary"] + sources["pec_insulator"], ["ApplyEfieldBoundary", "ApplyBfieldBoundary", "PEC_Insulator", "ApplyPEC_InsulatortoEfield", "ApplySilverMuellerBoundary"]),
