...
def main() -> int:
    ...
    checks = {
        ...
        "html_key_sections": all(marker in html_normalized for marker in ("如何阅读证据边界", "3.11.3 AMR subcycling 修改后的验证卡", "5.14.2.1 Vay 配置判读卡", "5.14.2.2 RZ implicit Villasenor 判读卡", "5.14.5.1 RZ 轴线判读卡", "收敛研究：描述性趋势不是正式阶数", "6.6.1 先按更新对象", "6.11.10 场求解器修改后的验证阶梯", "7.5.1 用正确的", "7.5.3 PML 配置与验证卡", "7.9.1 Transition-zone 判读卡", "8.14.3 修改诊断后的验证阶梯")),
        "pdf_page_count_positive": len(pdf_reader.pages) > 0,
        "pdf_key_sections": all(marker in pdf_text for marker in ("如何阅读证据边界", "AMR subcycling 修改后的验证卡", "Vay 配置判读卡", "RZ implicit Villasenor 判读卡", "RZ 轴线判读卡", "收敛研究：描述性趋势不是正式阶数", "先按更新对象理解 PSATD 系数", "场求解器修改后的验证阶梯", "用正确的 observable 判断 PML", "PML 配置与验证卡", "Transition-zone 判读卡", "修改诊断后的验证阶梯")),
