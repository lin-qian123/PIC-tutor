...
def main() -> int:
    ...
    foundation_openings = "\n".join(
        path.read_text(encoding="utf-8")[:2500] for path in reader_foundations
    )
    ...
    project_record_opening_markers = re.findall(
        r"v0\.\d+\s*(?:校准说明|的本章目标)|可审校长草稿|正式书稿版",
        foundation_openings,
    )
    checks = {
        ...
        "foundation_openings_are_reader_facing": not project_record_opening_markers,
