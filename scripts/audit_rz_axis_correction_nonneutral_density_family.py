#!/usr/bin/env python
"""Audit non-neutral RZ axis-correction shape behavior across ion densities."""
...
def main() -> int:
    ...
    checks = {
        "two_densities_present": densities == ["0.25", "0.5"],
        ...
        "species_ratio_density_stable": max(
            abs(species_ratios[densities[0]][str(shape)] - species_ratios[densities[1]][str(shape)])
            for shape in (1, 2, 3, 4)
        ) <= 1.0e-8,
        "quarter_total_rho_shape_monotonic": all(
            total_ratios["0.25"][str(shape)] > total_ratios["0.25"][str(shape + 1)]
            for shape in (1, 2, 3)
        ),
        ...
        "density_changes_total_rho_visibility": max(
            abs(total_ratios["0.25"][str(shape)] - total_ratios["0.5"][str(shape)])
            for shape in (2, 3, 4)
        ) > 1.0e-3,
    ...
    result = {
        "contract": "RZ axis correction non-neutral shape family across density",
        "classification": "RZ_NONNEUTRAL_AXIS_CORRECTION_TOTAL_RHO_CANCELLATION_DENSITY_SENSITIVE_BOUNDARY_OPEN",
        ...
        "scope": (
            "The species-level shape ratio is density-stable, while total-rho visibility changes with the ion "
            "density because species contributions can cancel at sampled axis cells. This narrows the boundary "
            "to a density-sensitive observable/cancellation effect without claiming a kernel root cause or closure."
        ),
    }
    args.output_dir.mkdir(parents=True, exist_ok=True)
    (args.output_dir / "contract.json").write_text(json.dumps(result, indent=2, ensure_ascii=False) + "\n", encoding="utf-8")
    lines = [
        "# RZ axis correction non-neutral shape family across density contract", "",
    ...
    lines.extend([
        "",
        "The species-level shape ratio is stable across densities. Total-rho is visible and shape-monotonic at `0.25*n0`, while `0.5*n0` shows cancellation for shapes 2-4; this is an open density-sensitive boundary, not a charge-closure or root-cause proof.",
    ])
