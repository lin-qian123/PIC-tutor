#!/usr/bin/env python
"""Generate a preflight report for staging the RZ JRhom first-stage patch."""
...
DEFAULT_REPORT_PATH = (
    ...
    / "code-reading"
    / "fieldsolver"
    / "rz_jrhom_first_stage_target_report.md"
)
AUDIT_SCRIPT = ROOT / "scripts" / "audit_rz_jrhom_first_stage_patch.py"
...
def render_next_steps(report: dict, warpx_root: Path) -> list[str]:
    ...
    stage_cmd = (
        f"python scripts/stage_rz_jrhom_first_stage_patch.py --warpx-root {warpx_root}"
    )
    preview_cmd = (
        f"python scripts/preview_rz_jrhom_first_stage_patch.py --warpx-root {warpx_root}"
    )
    dry_run_cmd = (
        f"python scripts/stage_rz_jrhom_first_stage_patch.py --warpx-root {warpx_root} --dry-run"
    )
    audit_cmd = (
        f"python scripts/audit_rz_jrhom_first_stage_patch.py --warpx-root {warpx_root}"
    )
    ...
    return [
        "- The target checkout already matches the current bundle.",
