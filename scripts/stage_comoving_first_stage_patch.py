#!/usr/bin/env python
"""Stage the generated comoving first-stage bundle into a target WarpX checkout.

This script intentionally lives in PIC-tutor. It does not assume the adjacent
../warpx checkout should be modified in-place unless the caller explicitly
passes that path.
"""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Copy the first-stage comoving helper into a WarpX tree and update CMake wiring."
    )
    parser.add_argument(
        "--warpx-root",
        type=Path,
...
def rewrite_cmake(text: str) -> str:
    ...
    if not replaced:
        raise ValueError(
            "did not find the expected analysis line for test_2d_comoving_psatd_hybrid"
        )
