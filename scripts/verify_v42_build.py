#!/usr/bin/env python
"""Verify the v0.42 Markdown/HTML/PDF artifact contract after building."""
...
MERGED_MARKDOWN = ROOT / "dist" / "pic-tutor-v0.42.md"
HTML = ROOT / "dist" / "pic-tutor-v0.42.html"
PDF = ROOT / "dist" / "pic-tutor-v0.42.pdf"
...
def main() -> None:
    ...
    checks = {
        "pdf_pages": len(reader.pages) == 307,
    ...
    print("v0.42 artifact verification")
    for name, passed in checks.items():
        print(f"  [{'PASS' if passed else 'FAIL'}] {name}")
    if not all(checks.values()):
        raise SystemExit("v0.42 artifact verification failed")
    print("[PASS] all v0.42 artifact checks")
