#!/usr/bin/env python
"""Verify the v0.47 Markdown/HTML/PDF artifact contract after building."""
...
MERGED_MARKDOWN = ROOT / "dist" / "pic-tutor-v0.47.md"
HTML = ROOT / "dist" / "pic-tutor-v0.47.html"
PDF = ROOT / "dist" / "pic-tutor-v0.47.pdf"
...
def main() -> None:
    ...
    checks = {
        "pdf_pages": len(reader.pages) == 309,
    ...
    print("v0.47 artifact verification")
    for name, passed in checks.items():
        print(f"  [{'PASS' if passed else 'FAIL'}] {name}")
    if not all(checks.values()):
        raise SystemExit("v0.47 artifact verification failed")
    print("[PASS] all v0.47 artifact checks")
