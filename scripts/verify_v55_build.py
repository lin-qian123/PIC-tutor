#!/usr/bin/env python
"""Verify the v0.55 Markdown/HTML/PDF artifact contract after building."""
...
MERGED_MARKDOWN = ROOT / "dist" / "pic-tutor-v0.55.md"
HTML = ROOT / "dist" / "pic-tutor-v0.55.html"
PDF = ROOT / "dist" / "pic-tutor-v0.55.pdf"
...
def main() -> None:
    ...
    print("v0.55 artifact verification")
    for name, passed in checks.items():
        print(f"  [{'PASS' if passed else 'FAIL'}] {name}")
    if not all(checks.values()):
        raise SystemExit("v0.55 artifact verification failed")
    print("[PASS] all v0.55 artifact checks")
