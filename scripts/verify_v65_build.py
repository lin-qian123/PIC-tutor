#!/usr/bin/env python
"""Verify the v0.65 Markdown/HTML/PDF artifact contract after building."""
...
MERGED_MARKDOWN = ROOT / "dist" / "pic-tutor-v0.65.md"
HTML = ROOT / "dist" / "pic-tutor-v0.65.html"
PDF = ROOT / "dist" / "pic-tutor-v0.65.pdf"
EXPECTED_PDF_PAGES = 311
...
def main() -> None:
    ...
    print("v0.65 artifact verification")
    for name, passed in checks.items():
        print(f"  [{'PASS' if passed else 'FAIL'}] {name}")
    if not all(checks.values()):
        raise SystemExit("v0.65 artifact verification failed")
    print("[PASS] all v0.65 artifact checks")
