#!/usr/bin/env python
"""Verify the v0.66 Markdown/HTML/PDF artifact contract after building."""
...
MERGED_MARKDOWN = ROOT / "dist" / "pic-tutor-v0.66.md"
HTML = ROOT / "dist" / "pic-tutor-v0.66.html"
PDF = ROOT / "dist" / "pic-tutor-v0.66.pdf"
...
def main() -> None:
    ...
    print("v0.66 artifact verification")
    for name, passed in checks.items():
        print(f"  [{'PASS' if passed else 'FAIL'}] {name}")
    if not all(checks.values()):
        raise SystemExit("v0.66 artifact verification failed")
    print("[PASS] all v0.66 artifact checks")
