#!/usr/bin/env python
"""Verify the v0.70 Markdown/HTML/PDF artifact contract after building."""
...
MERGED_MARKDOWN = ROOT / "dist" / "pic-tutor-v0.70.md"
HTML = ROOT / "dist" / "pic-tutor-v0.70.html"
PDF = ROOT / "dist" / "pic-tutor-v0.70.pdf"
EXPECTED_PDF_PAGES = 326
...
def main() -> None:
    ...
    print("v0.70 artifact verification")
    for name, passed in checks.items():
        print(f"  [{'PASS' if passed else 'FAIL'}] {name}")
    if not all(checks.values()):
        raise SystemExit("v0.70 artifact verification failed")
    print("[PASS] all v0.70 artifact checks")
