#!/usr/bin/env python
"""Verify the v0.71 Markdown/HTML/PDF artifact contract after building."""
...
MERGED_MARKDOWN = ROOT / "dist" / "pic-tutor-v0.71.md"
HTML = ROOT / "dist" / "pic-tutor-v0.71.html"
PDF = ROOT / "dist" / "pic-tutor-v0.71.pdf"
EXPECTED_PDF_PAGES = 327
...
def main() -> None:
    ...
    print("v0.71 artifact verification")
    for name, passed in checks.items():
        print(f"  [{'PASS' if passed else 'FAIL'}] {name}")
    if not all(checks.values()):
        raise SystemExit("v0.71 artifact verification failed")
    print("[PASS] all v0.71 artifact checks")
