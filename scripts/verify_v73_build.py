#!/usr/bin/env python
"""Verify the v0.73 Markdown/HTML/PDF artifact contract after building."""
...
MERGED_MARKDOWN = ROOT / "dist" / "pic-tutor-v0.73.md"
HTML = ROOT / "dist" / "pic-tutor-v0.73.html"
PDF = ROOT / "dist" / "pic-tutor-v0.73.pdf"
EXPECTED_PDF_PAGES = 328
...
def main() -> None:
    ...
    print("v0.73 artifact verification")
    for name, passed in checks.items():
        print(f"  [{'PASS' if passed else 'FAIL'}] {name}")
    if not all(checks.values()):
        raise SystemExit("v0.73 artifact verification failed")
    print("[PASS] all v0.73 artifact checks")
