#!/usr/bin/env python
"""Verify the v0.76 Markdown/HTML/PDF artifact contract after building."""
...
MERGED_MARKDOWN = ROOT / "dist" / "pic-tutor-v0.76.md"
HTML = ROOT / "dist" / "pic-tutor-v0.76.html"
PDF = ROOT / "dist" / "pic-tutor-v0.76.pdf"
...
def main() -> None:
    ...
    print("v0.76 artifact verification")
    for name, passed in checks.items():
        print(f"  [{'PASS' if passed else 'FAIL'}] {name}")
    if not all(checks.values()):
        raise SystemExit("v0.76 artifact verification failed")
    print("[PASS] all v0.76 artifact checks")
