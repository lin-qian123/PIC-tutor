#!/usr/bin/env python
"""Verify the v0.81 Markdown/HTML/PDF artifact contract after building."""
...
MERGED_MARKDOWN = ROOT / "dist" / "pic-tutor-v0.81.md"
HTML = ROOT / "dist" / "pic-tutor-v0.81.html"
PDF = ROOT / "dist" / "pic-tutor-v0.81.pdf"
...
def main() -> None:
    ...
    checks = {
        ...
        "editorial_quality_audit": all(
            marker in source
            for marker in (
                "AUTOMATED_EDITORIAL_AUDIT_PASS_MANUAL_REVIEW_OPEN",
                "人工通读",
                "公开再分发",
            )
        ),
    ...
    print("v0.81 artifact verification")
    for name, passed in checks.items():
        print(f"  [{'PASS' if passed else 'FAIL'}] {name}")
    if not all(checks.values()):
        raise SystemExit("v0.81 artifact verification failed")
    print("[PASS] all v0.81 artifact checks")
