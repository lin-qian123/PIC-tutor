#!/usr/bin/env python
"""Verify the v0.87 Markdown/HTML/PDF artifact contract after building."""
...
MERGED_MARKDOWN = ROOT / "dist" / "pic-tutor-v0.87.md"
HTML = ROOT / "dist" / "pic-tutor-v0.87.html"
PDF = ROOT / "dist" / "pic-tutor-v0.87.pdf"
MANUAL_SPOTCHECK = ROOT / "docs" / "manual-editorial-spotcheck-v0.87.md"
EXPECTED_PDF_PAGES = 330
...
def main() -> None:
    ...
    print("v0.87 artifact verification")
    for name, passed in checks.items():
        print(f"  [{'PASS' if passed else 'FAIL'}] {name}")
    if not all(checks.values()):
        raise SystemExit("v0.87 artifact verification failed")
    print("[PASS] all v0.87 artifact checks")
