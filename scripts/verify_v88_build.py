#!/usr/bin/env python
"""Verify the v0.88 Markdown/HTML/PDF artifact contract after building."""
...
MERGED_MARKDOWN = ROOT / "dist" / "pic-tutor-v0.88.md"
HTML = ROOT / "dist" / "pic-tutor-v0.88.html"
PDF = ROOT / "dist" / "pic-tutor-v0.88.pdf"
MANUAL_SPOTCHECK = ROOT / "docs" / "manual-editorial-spotcheck-v0.88.md"
...
def main() -> None:
    ...
    print("v0.88 artifact verification")
    for name, passed in checks.items():
        print(f"  [{'PASS' if passed else 'FAIL'}] {name}")
    if not all(checks.values()):
        raise SystemExit("v0.88 artifact verification failed")
    print("[PASS] all v0.88 artifact checks")
