#!/usr/bin/env python
"""Verify the v0.89 Markdown/HTML/PDF artifact contract after building."""
...
MERGED_MARKDOWN = ROOT / "dist" / "pic-tutor-v0.89.md"
HTML = ROOT / "dist" / "pic-tutor-v0.89.html"
PDF = ROOT / "dist" / "pic-tutor-v0.89.pdf"
MANUAL_SPOTCHECK = ROOT / "docs" / "manual-editorial-spotcheck-v0.89.md"
...
def main() -> None:
    ...
    checks = {
        ...
        "manual_editorial_spotcheck": all(
            ...
            for marker in (
                "# v0.89 PDF manual editorial spotcheck",
                "| 184 |",
                "| 190 |",
                "新增段落与路径均在版心内",
    ...
    print("v0.89 artifact verification")
    for name, passed in checks.items():
        print(f"  [{'PASS' if passed else 'FAIL'}] {name}")
    if not all(checks.values()):
        raise SystemExit("v0.89 artifact verification failed")
    print("[PASS] all v0.89 artifact checks")
