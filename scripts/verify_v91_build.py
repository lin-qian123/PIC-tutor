#!/usr/bin/env python
"""Verify the v0.91 Markdown/HTML/PDF artifact contract after building."""
...
MERGED_MARKDOWN = ROOT / "dist" / "pic-tutor-v0.91.md"
HTML = ROOT / "dist" / "pic-tutor-v0.91.html"
PDF = ROOT / "dist" / "pic-tutor-v0.91.pdf"
MANUAL_SPOTCHECK = ROOT / "docs" / "manual-editorial-spotcheck-v0.91.md"
...
def main() -> None:
    ...
    checks = {
        ...
        "manual_editorial_spotcheck": all(
            ...
            for marker in (
                "# v0.91 PDF manual editorial spotcheck",
                "| 184 |",
                "| 190 |",
                "新增段落与路径均在版心内",
    ...
    print("v0.91 artifact verification")
    for name, passed in checks.items():
        print(f"  [{'PASS' if passed else 'FAIL'}] {name}")
    if not all(checks.values()):
        raise SystemExit("v0.91 artifact verification failed")
    print("[PASS] all v0.91 artifact checks")
