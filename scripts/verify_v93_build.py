#!/usr/bin/env python
"""Verify the v0.93 Markdown/HTML/PDF artifact contract after building."""
...
MERGED_MARKDOWN = ROOT / "dist" / "pic-tutor-v0.93.md"
HTML = ROOT / "dist" / "pic-tutor-v0.93.html"
PDF = ROOT / "dist" / "pic-tutor-v0.93.pdf"
MANUAL_SPOTCHECK = ROOT / "docs" / "manual-editorial-spotcheck-v0.93.md"
EXPECTED_PDF_PAGES = 331
...
def main() -> None:
    ...
    checks = {
        ...
        "rz_axis_charge_repeat_stability": all(
            ...
            for marker in (
                ...
                "rz-axis-charge-repeat-stability-v0.93/contract.{json,md}",
        ...
        "manual_editorial_spotcheck": all(
            ...
            for marker in (
                "# v0.93 PDF manual editorial spotcheck",
                "| 184 |",
                "| 191 |",
                "新增段落与路径均在版心内",
    ...
    print("v0.93 artifact verification")
    for name, passed in checks.items():
        print(f"  [{'PASS' if passed else 'FAIL'}] {name}")
    if not all(checks.values()):
        raise SystemExit("v0.93 artifact verification failed")
    print("[PASS] all v0.93 artifact checks")
