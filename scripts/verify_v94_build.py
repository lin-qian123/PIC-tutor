#!/usr/bin/env python
"""Verify the v0.94 Markdown/HTML/PDF artifact contract after building."""
...
MERGED_MARKDOWN = ROOT / "dist" / "pic-tutor-v0.94.md"
HTML = ROOT / "dist" / "pic-tutor-v0.94.html"
PDF = ROOT / "dist" / "pic-tutor-v0.94.pdf"
MANUAL_SPOTCHECK = ROOT / "docs" / "manual-editorial-spotcheck-v0.94.md"
...
def main() -> None:
    ...
    print("v0.94 artifact verification")
    for name, passed in checks.items():
        print(f"  [{'PASS' if passed else 'FAIL'}] {name}")
    if not all(checks.values()):
        raise SystemExit("v0.94 artifact verification failed")
    print("[PASS] all v0.94 artifact checks")
