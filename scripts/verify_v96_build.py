#!/usr/bin/env python
"""Verify the v0.96 Markdown/HTML/PDF artifact contract after building."""
...
MERGED_MARKDOWN = ROOT / "dist" / "pic-tutor-v0.96.md"
HTML = ROOT / "dist" / "pic-tutor-v0.96.html"
PDF = ROOT / "dist" / "pic-tutor-v0.96.pdf"
MANUAL_SPOTCHECK = ROOT / "docs" / "manual-editorial-spotcheck-v0.96.md"
...
def main() -> None:
    ...
    print("v0.96 artifact verification")
    for name, passed in checks.items():
        print(f"  [{'PASS' if passed else 'FAIL'}] {name}")
    if not all(checks.values()):
        raise SystemExit("v0.96 artifact verification failed")
    print("[PASS] all v0.96 artifact checks")
