#!/usr/bin/env python
"""Verify the v0.97 Markdown/HTML/PDF artifact contract after building."""
...
MERGED_MARKDOWN = ROOT / "dist" / "pic-tutor-v0.97.md"
HTML = ROOT / "dist" / "pic-tutor-v0.97.html"
PDF = ROOT / "dist" / "pic-tutor-v0.97.pdf"
MANUAL_SPOTCHECK = ROOT / "docs" / "manual-editorial-spotcheck-v0.97.md"
...
def main() -> None:
    ...
    checks = {
        ...
        "manual_editorial_spotcheck": all(
            ...
            for marker in (
                "# v0.97 PDF manual editorial spotcheck",
                "| 184 |",
                "| 191 |",
                "新增段落与路径均在版心内",
        ...
        "transition_zone_runtime_activation": all(
            ...
            for marker in (
                ...
                "transition-zone-runtime-activation-v0.97/contract.{json,md}",
    ...
    print("v0.97 artifact verification")
    for name, passed in checks.items():
        print(f"  [{'PASS' if passed else 'FAIL'}] {name}")
    if not all(checks.values()):
        raise SystemExit("v0.97 artifact verification failed")
    print("[PASS] all v0.97 artifact checks")
