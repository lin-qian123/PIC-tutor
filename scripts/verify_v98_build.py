#!/usr/bin/env python
"""Verify the v0.98 Markdown/HTML/PDF artifact contract after building."""
...
MERGED_MARKDOWN = ROOT / "dist" / "pic-tutor-v0.98.md"
HTML = ROOT / "dist" / "pic-tutor-v0.98.html"
PDF = ROOT / "dist" / "pic-tutor-v0.98.pdf"
MANUAL_SPOTCHECK = ROOT / "docs" / "manual-editorial-spotcheck-v0.98.md"
EXPECTED_PDF_PAGES = 333
...
def main() -> None:
    ...
    checks = {
        ...
        "transition_zone_runtime_activation": all(
            ...
            for marker in (
                ...
                "transition-zone-runtime-activation-v0.98/contract.{json,md}",
    ...
    print("v0.98 artifact verification")
    for name, passed in checks.items():
        print(f"  [{'PASS' if passed else 'FAIL'}] {name}")
    if not all(checks.values()):
        raise SystemExit("v0.98 artifact verification failed")
    print("[PASS] all v0.98 artifact checks")
