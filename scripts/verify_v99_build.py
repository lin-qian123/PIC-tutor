#!/usr/bin/env python
"""Verify the v0.99 Markdown/HTML/PDF artifact contract after building."""
...
MERGED_MARKDOWN = ROOT / "dist" / "pic-tutor-v0.99.md"
HTML = ROOT / "dist" / "pic-tutor-v0.99.html"
PDF = ROOT / "dist" / "pic-tutor-v0.99.pdf"
MANUAL_SPOTCHECK = ROOT / "docs" / "manual-editorial-spotcheck-v0.99.md"
...
def main() -> None:
    ...
    checks = {
        ...
        "transition_zone_runtime_activation": all(
            ...
            for marker in (
                ...
                "transition-zone-runtime-activation-v0.99/contract.{json,md}",
    ...
    print("v0.99 artifact verification")
    for name, passed in checks.items():
        print(f"  [{'PASS' if passed else 'FAIL'}] {name}")
    if not all(checks.values()):
        raise SystemExit("v0.99 artifact verification failed")
    print("[PASS] all v0.99 artifact checks")
